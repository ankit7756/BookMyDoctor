...
const DoctorsList = () => {
  ...
  if (loading) {
    return (
      <motion.div
        className="m-5 flex justify-center items-center h-[60vh]"
        initial="hidden"
        animate="visible"
        variants={fadeIn}
      >
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <p className="text-base md:text-lg text-gray-600 max-w-2xl mx-auto mt-6">
            View and manage all doctors—ensure seamless healthcare operations with our trusted specialists.
          </p>
        </motion.div>
        ...
        >
          {!doctors || doctors.length === 0 ? (
            ...
            <motion.div
              className="flex w-full flex-wrap gap-6 pt-5"
              variants={staggerContainer}
            >
              {doctors.map((doctor) => (
                <motion.div
                  key={doctor.id}
                  className="border border-gray-200 rounded-2xl max-w-sm overflow-hidden cursor-pointer hover:shadow-xl transition-all duration-500 relative group"
                  variants={scaleUp}
                  whileHover={{ scale: 1.02 }}
                  transition={{ duration: 0.3 }}
                >
                  {/* Gradient Glow on Hover */}
                  <div className="pointer-events-none absolute -inset-1 bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-500 rounded-2xl blur opacity-0 group-hover:opacity-20 transition-opacity duration-300"></div>


                  <img
                    className="bg-indigo-50 h-48 w-full object-cover group-hover:bg-opacity-80 transition-all duration-500"
                    src={doctor.image}
                    alt={doctor.name}
                  />
                  <div className="p-4 bg-white">
                    <p className="text-gray-900 text-lg font-medium line-clamp-1">{doctor.name}</p>
                    <p className="text-gray-600 text-sm line-clamp-1">{doctor.speciality}</p>

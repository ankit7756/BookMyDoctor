...
const RelatedDoctors = ({ speciality, docId }) => {
    ...
    useEffect(() => {
        if (doctors.length > 0 && speciality) {
            const doctorsData = doctors.filter((doc) => doc.speciality === speciality && doc.id !== parseInt(docId));
        ...
        >
            ...
            <div className="container mx-auto px-4 relative z-10">
                <motion.div
                    ...
                    variants={fadeIn}
                >
                    <span className="bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent text-sm font-extrabold tracking-wide uppercase px-4 py-1.5 rounded-full mb-4 inline-block bg-indigo-50 shadow-sm border border-indigo-100">Trusted Specialists</span>
                    ...
                    <p className="text-base md:text-lg text-gray-600 max-w-2xl mx-auto mt-6">
                        Explore our curated list of trusted specialists—book an appointment with ease for your health needs.
                    </p>
                </motion.div>
                    ...
                    <div className="w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 pt-5 gap-y-6 px-3 sm:px-0">
                        {relDoc.slice(0, 5).map((item, index) => (
                            <motion.div
                                ...
                                className="border border-gray-200 rounded-2xl overflow-hidden cursor-pointer hover:shadow-xl transition-all duration-500 relative group"
                                key={index}
                                variants={scaleUp}
                            ...
                            >
                                ...
                                <div className="absolute -inset-1 bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-500 rounded-2xl blur opacity-0 group-hover:opacity-20 transition-opacity duration-300"></div>
                                ...
                                <div className="p-4 bg-white">
                                    ...
                                    <p className="text-gray-900 text-lg font-medium mt-2 line-clamp-1">{item.name}</p>
                                    <p className="text-gray-600 text-sm line-clamp-1">{item.speciality}</p>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                ...
                <motion.button
                    ...
                    className="mx-auto block bg-gradient-to-r from-indigo-600 to-blue-600 text-white px-12 py-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
                    initial="hidden"
                    animate="visible"
                    variants={fadeIn}
                    whileHover={{ scale: 1.05 }}
